...
async def add_emoji(ctx: interactions.CommandContext, **kwargs):
    ...
    if not validate_emoji_name(emoji_name):
        await ctx.send("Emoji name must be alphanumeric characters and underscores only")
        await ctx.delete()
        return
    
    if not validate_image_url(emoji_url):
        await ctx.send("Invalid image URL, url must end in png, jpg, jpeg, gif, or webp")
        await ctx.delete()
        return
